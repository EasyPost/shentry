...
@pytest.mark.parametrize(
    "argv,expected_full_command,expected_command_ws,expected_shell",
    (
...
def test_get_command(
    mocker, argv, expected_full_command, expected_command_ws, expected_shell
):
    mocker.patch("os.environ", autospec=True)
    os.environ.get.return_value = "/bin/bash"
    assert shentry.get_command(argv) == (
...
class TestSimpleSentryClient(object):
    def test_new_from_environment(self, mocker):
        mocker.patch.dict(
            "os.environ", {"SHELL_SENTRY_DSN": "https://pub:priv@sentry.test/1"}
        )
    ...
    def test_new_from_environment_regular_dsn(self, mocker):
        mocker.patch.dict(
            "os.environ", {"SENTRY_DSN": "https://pub:priv@sentry.test/3"}
        )
        client = shentry.SimpleSentryClient.new_from_environment()
    ...
    def test_new_from_environment_with_file(self, mocker):
        ...
        mocker.patch.object(
            shentry,
            "read_systemwide_config",
            return_value="https://pub:priv@sentry.test/2",
        )
        client = shentry.SimpleSentryClient.new_from_environment()
...
def test_main(mocker, tmpdir):
    mock_client = Mock(autospec=shentry.SimpleSentryClient)
    mocker.patch(
        "shentry.SimpleSentryClient.new_from_environment", return_value=mock_client
    )
    mocker.patch("tempfile.mkdtemp", return_value=str(tmpdir))
    mocker.patch.dict(
        "os.environ", {"SHELL": "/bin/fish", "PATH": "A_PATH", "TZ": "UTC"}
    )
    mock_popen = Mock(autospec=subprocess.Popen)
